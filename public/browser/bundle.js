...
Board.prototype.clearPath = function(clickedButton) {
  ...
  document.getElementById("startButtonStart").onclick = () => {
    if (!this.currentAlgorithm) {
      ...
      this.toggleButtons();
      let weightedAlgorithms = ["dijkstra"];
      let unweightedAlgorithms = ["dfs", "bfs"];
      let success;
...
Board.prototype.changeStartNodeImages = function() {
  ...
  if (unweighted.includes(this.currentAlgorithm)) {
    ...
    if (this.currentAlgorithm === "dijkstra" ) {
      document.getElementById("algorithmDescriptor").innerHTML = `${name} is <i><b>weighted</b></i> and <i><b> guarantee</b></i> the shortest path when the edge costs are all positive!`;
    }
